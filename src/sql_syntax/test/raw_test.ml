...
let%test_unit "make raw data" =
  let node =
    R.make_node B ~layouts:[ R.make_leaf A ~leading:(T.leading []) ~trailing:(T.trailing []) ~token:Tok_colon ]
  in
  assert (R.match' (Types.Token.equal Tok_colon) node = false)

let%test_unit "make leaf data" =
  let node = R.make_leaf A ~leading:(T.leading []) ~trailing:(T.trailing []) ~token:Tok_colon in
...
let%expect_test "raw to string" =
  ...
  @@ R.make_node B
       ~layouts:
         [ R.make_leaf A ~leading:(T.leading []) ~trailing:(T.trailing [ Tok_space ]) ~token:(Tok_ident "ident")
         ; R.make_leaf A ~leading:(T.leading []) ~trailing:(T.trailing [ Tok_space ]) ~token:Op_eq
         ; R.make_leaf A ~leading:(T.leading []) ~trailing:(T.trailing []) ~token:(Tok_string "foo")
         ];

...
module type S = sig
  ...
  val map : rewriter:R.t option rewriter -> R.t -> R.t list

  (** [when_leaf leaf rewriter ~options raw] applies [rewriter] if [raw] is same kind of [leaf]. *)
  val when_leaf : K.leaf -> R.t option rewriter -> R.t -> R.t option

include (
  struct
    ...
    module P (D : Data) = struct
      ...
      let unbounded_preceding =
        M.bump_kw Kw.Kw_unbounded *> M.bump_kw Kw.Kw_preceding

      let unbounded_following =
        M.bump_kw Kw.Kw_unbounded *> M.bump_kw Kw.Kw_following
      ...
      let between =
        ...
        let between_1 =
          ...
          <|> (M.skip >>= D.expr)
              *> (M.bump_kw Kw.Kw_preceding <|> M.bump_kw Kw.Kw_following)
        in
        let between_2 =
      ...
      let frame_spec =
        let* () =
          M.bump_kw Kw.Kw_range <|> M.bump_kw Kw.Kw_rows
          <|> M.bump_kw Kw.Kw_groups
        in
        let* () =
          current_row <|> unbounded_preceding <|> between
          <|> (M.skip >>= D.expr) *> M.bump_kw Kw.Kw_preceding
        in
        let* () =
          ...
          M.bump_kw Kw.Kw_exclude
          *> (no_other <|> current_row <|> group <|> ties)
          <|> M.skip
        in
        M.return ()

...
let%expect_test "parse " =
  ...
  Util.run {| some_table as not materialized (select a,b,c from table_c) |} p |> print_endline;
  Util.run {| some_table (c1,c2,c3) as (select 1) |} p |> print_endline;
  ...
  [%expect
    ...
    some_table (c1,c2,c3) as (select 1) |}]

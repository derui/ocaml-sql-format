...
let%expect_test "select_10 for formatting" =
  ...
  [%expect
    {|
      SELECT
          COUNT(*) OVER () AS v,TEXTAGG (FOR a AS t1, b AS t2 DELIMITER ',') OVER (PARTITION BY e) AS v2,
          COUNT(*) OVER (PARTITION BY expr ORDER BY bc) AS v3,COUNT(*) FILTER (WHERE a = 3) AS v4,
          COUNT(*) FILTER (WHERE b > 4) OVER ( RANGE CURRENT ROW) AS v5,COUNT(*) OVER ( RANGE UNBOUNDED FOLLOWING) AS v6,
          COUNT(*) OVER ( RANGE 3 PRECEDING) AS v7,COUNT(*) OVER ( RANGE BETWEEN UNBOUNDED FOLLOWING AND UNBOUNDED PRECEDING) AS v8,
          COUNT(*) OVER ( ROWS UNBOUNDED FOLLOWING) AS v9,COUNT(*) OVER ( ROWS 3 PRECEDING) AS v10,
          COUNT(*) OVER ( ROWS BETWEEN UNBOUNDED FOLLOWING AND UNBOUNDED PRECEDING) AS v11
      FROM
          a,
          b |}]

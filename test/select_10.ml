    ...
    {|
      SELECT
          ...
          COUNT(*) OVER (PARTITION BY expr ORDER BY
              bc) AS v3,COUNT(*) FILTER (WHERE a = 3) AS v4,COUNT(*) FILTER (WHERE
                                                            b > 4) OVER ( RANGE CURRENT ROW) AS v5,
          COUNT(*) OVER ( RANGE UNBOUNDED FOLLOWING) AS v6,COUNT(*) OVER ( RANGE 3 PRECEDING) AS v7,
          COUNT(*) OVER ( RANGE BETWEEN UNBOUNDED FOLLOWING AND UNBOUNDED PRECEDING) AS v8,
